       ...
       /* [HttpGet]
        public async Task<ActionResult> DisputaGeralAsync()
        {
            try
            {
                ...
                string uriBasePersonagens = "http://lzsouza.somee.com/RpgApi/Personagens/GetAll";
                HttpResponseMessage response = await httpClient.GetAsync(uriBasePersonagens);

                string serialized = await response.Content.ReadAsStringAsync();

                List<PersonagemViewModel> listaPersonagens = await Task(() =>
                    JsonConvert.DeserializeObject<List<PersonagemViewModel>>(serialized));

                string uriDisputa = "http://lzsouza.somee.com/RpgApi/Disputa/EmGrupo";
                DisputaViewModel disputa = new DisputaViewModel();
                disputa.ListaIdPersonagens = new List<int>();
                disputas.ListaIdPersonagens.AddRange(listaPersonagens.Select(p => p.Id));
                ...
                if(response.StatusCode == System.Net.HttpStatusCode.OK)
                {
                    ...
                    TempData["Mensagem"] = string.Join("<br/>", "Personagens");
